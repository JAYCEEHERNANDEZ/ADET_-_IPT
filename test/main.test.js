...
describe("login", () => {
  it("should return success message if the email and password is correct", () => {
    expect(login("juandelacruz@email.com", "Str0ngp@ssword")).toEqual("Login Successful");
  });

  it("should throw an error if email do not have @", () => {
    expect(() => {
      login("hernandezemail.com", "Str0ngp@ssword");
    }).toThrow("Invalid email");
  });

  it("should throw an error if email do not have domain", () => {
    expect(() => {
      login("hernandezemail@email", "Str0ngp@ssword");
    }).toThrow("Invalid email");
  });
  ...
  it("should throw an error if password is empty", () => {
    expect(() => {
      login("juandelacruz@email.com", "");
    }).toThrow("Weak Password");
  });

  it("should throw an error if password is null", () => {
    expect(() => {
      login("juandelacruz@email.com", null);
    }).toThrow("Weak Password");
  });

 it("should return warning message when email is incorrect", () => {
  expect( login("hernandez@email.com", "Str0ngp@ssword")).toEqual("Incorrect email or password");
  });

  it("should return warning message when password is incorrect", () => {
  expect(login("juandelacruz@email.com", "Hampaslupa@123")).toEqual("Incorrect email or password");
  });
